...
type Screen = "dashboard" | "groups" | "profile";
type DashboardData = {
  user: User;
      ...
      {selectedEventId ? (
        ...
        <>
          <View style={styles.tabs}>
            {(["dashboard", "groups", "profile"] as Screen[]).map((item) => (
...
function ProfileScreen({ user, onSaved }: { user: User; onSaved: (user: User) => void }) {
  ...
  useEffect(() => {
    void api<{ user: User; preference: Preference | null }>("/api/mobile/me").then((body) => {
      if (body.preference) setForm({ ...body.preference, name: body.user.name });
      setLoaded(true);
    });
  }, []);
  ...
  async function save() {
    ...
    try {
      const body = await api<{ user: User; preference: Preference }>("/api/mobile/onboarding", {
        method: "PUT",
        body: JSON.stringify({ ...form, name: form.name || user.name })
      });
      onSaved(body.user);
  ...
  return (
    <ScrollView style={styles.body}>
      ...
      <Input label="Name" value={form.name ?? ""} onChangeText={(value) => setForm((current) => ({ ...current, name: value }))} />
      <Input label="City" value={form.city} onChangeText={(value) => setForm((current) => ({ ...current, city: value }))} />
      <Input label="Neighborhood" value={form.neighborhood} onChangeText={(value) => setForm((current) => ({ ...current, neighborhood: value }))} />
      <ChoiceGroup label="Goals" options={goals} selected={form.goals} onChange={(values) => setForm((current) => ({ ...current, goals: values }))} />
      <ChoiceGroup label="Activities" options={activities} selected={form.activities} onChange={(values) => setForm((current) => ({ ...current, activities: values }))} />
      <ChoiceGroup label="Availability" options={availability} selected={form.availability} onChange={(values) => setForm((current) => ({ ...current, availability: values }))} />
      <Input label="Current level" value={form.currentLevel} onChangeText={(value) => setForm((current) => ({ ...current, currentLevel: value }))} />
      <Input label="Target goal" value={form.targetGoal} onChangeText={(value) => setForm((current) => ({ ...current, targetGoal: value }))} />
      <Input label="Vibe" value={form.vibe} onChangeText={(value) => setForm((current) => ({ ...current, vibe: value }))} />
      <Input label="Comfort preference" value={form.comfortPreference} onChangeText={(value) => setForm((current) => ({ ...current, comfortPreference: value }))} />
...
function Input(props: { label: string; value: string; onChangeText: (value: string) => void; secureTextEntry?: boolean; autoCapitalize?: "none"; keyboardType?: "email-address"; multiline?: boolean }) {
...
function ChoiceGroup({ label, options, selected, onChange }: { label: string; options: string[]; selected: string[]; onChange: (values: string[]) => void }) {
  return (
    <View style={styles.field}>
      <Text style={styles.label}>{label}</Text>
      <View style={styles.chips}>
